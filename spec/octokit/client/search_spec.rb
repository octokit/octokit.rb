...
describe Octokit::Client::Search do
  ...
  describe '.search_code' do
    it 'searches code', :vcr do
      ...
      assert_requested :get,
                       github_url('/search/code?q=code%20user:github%20in:file%20extension:gemspec%20-repo:octokit/octokit.rb&sort=indexed&order=asc')
  ...
  describe '.search_commits' do
    it 'searches commits', :vcr do
      ...
      assert_requested :get,
                       github_url('/search/commits?q=repo:octokit/octokit.rb%20author:jasonrudolph&sort=author-date&order=asc')
      expect(results.total_count).to be_kind_of Integer
      expect(results.items).to be_kind_of Array
    end
  end
