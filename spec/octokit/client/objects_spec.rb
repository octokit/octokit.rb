  ...
  describe '.tree', :vcr do
    ...
    it 'gets a tree recursively' do
      ...
      assert_requested :get,
                       github_url('/repos/sferik/rails_admin/git/trees/3cdfabd973bc3caac209cba903cfdb3bf6636bcd?recursive=true')
    end
  end

  describe '.create_tree', :vcr do
    it 'creates a tree' do
      @client.create_tree(@test_repo,
                          [{ 'path' => 'wynning.rb', 'mode' => '100644', 'type' => 'blob',
                             :content => "require 'fun'" }])
