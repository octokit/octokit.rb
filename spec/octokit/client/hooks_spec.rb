...
describe Octokit::Client::Hooks do
  ...
  context 'with repository' do
    ...
    context 'with hook' do
      before(:each) do
        @hook = @client.create_hook(@repo.full_name, 'railsbp',
                                    { railsbp_url: 'http://railsbp.com', token: 'xAAQZtJhYHGagsed1kYR' })
      end
      ...
      describe '.edit_hook', :vcr do
        it 'edits a hook' do
          @client.edit_hook(@repo.full_name, @hook.id, 'railsbp',
                            { railsbp_url: 'https://railsbp.com', token: 'xAAQZtJhYHGagsed1kYR' })
      ...
      describe '.test_hook', :vcr do
        it 'tests a hook' do
          @client.create_hook(@repo.full_name, 'railsbp',
                              { railsbp_url: 'http://railsbp.com', token: 'xAAQZtJhYHGagsed1kYR' })
  ...
  context 'with org hook' do
    ...
    describe '.edit_org_hook', :vcr do
      it 'edits an org hook' do
        @client.edit_org_hook(test_github_org, @org_hook.id,
                              { url: 'https://railsbp.com', content_type: 'application/json' })
