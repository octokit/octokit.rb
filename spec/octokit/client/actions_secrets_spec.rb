...
describe Octokit::Client::ActionsSecrets do
  ...
  context 'with an org' do
    describe '.get_org_actions_public_key', :vcr do
      it 'get org public key for secrets encryption' do
        box = create_box(@client.get_org_actions_public_key(test_github_org))
        expect(box[:key_id]).not_to be_empty
      end
    end # .get_org_actions_public_key
  end

  context 'with an org  without secrets' do
    describe '.list_org_actions_secrets', :vcr do
      it 'returns empty list of secrets' do
        secrets = @client.list_org_actions_secrets(test_github_org)
        expect(secrets.total_count).to eq(0)
        expect(secrets.secrets).to be_empty
      end
    end # .list_org_actions_secrets

    describe '.create_or_update_org_actions_secret', :vcr do
      it 'creating secret returns 201' do
        box = create_box(@client.get_org_actions_public_key(test_github_org))
        encrypted = box[:box].encrypt(@secrets.first[:value])
        @client.create_or_update_org_actions_secret(
    ...
    end # .create_or_update_org_actions_secret
  end
  ...
  context 'with an org with a secret' do
    ...
    describe '.get_org_actions_secret', :vcr do
      it 'return timestamps related to one secret' do
        received = @client.get_org_actions_secret(test_github_org, @secrets.first[:name])
        expect(received.name).to eq(@secrets.first[:name].upcase)
      end
    end # .get_org_actions_secret
    ...
    describe '.delete_org_actions_secret', :vcr do
      it 'delete existing secret' do
        @client.delete_org_actions_secret(test_github_org, @secrets.first[:name])
        expect(@client.last_response.status).to eq(204)
      end
    end # .delete_org_actions_secret
  end
end
