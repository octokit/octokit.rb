...
describe Octokit::Client::SourceImport do
  ...
  describe 'post deprecation' do
    before(:each) do
      @client.start_source_import(@repo.full_name, 'https://bitbucket.org/spraints/goboom',
                                  { vcs: 'hg', accept: preview_header })
    end
    ...
    describe '.map_source_import_commit_author', :vcr do
      it 'updates the commit authors identity' do
        ...
        sleep 1 while @client.source_import_commit_authors(@repo.full_name, accept: preview_header).empty?
