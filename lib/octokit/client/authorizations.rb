module Octokit
  class Client
    module Authorizations
      ...
      def authorizations
        root.rels[:authorizations].get.data
      end
      ...
      def authorization(number)
        root.rels[:authorizations].get(:uri => { :authorization_id => number }).data
      end
      ...
      def create_authorization(options={})
        ...
        root.rels[:authorizations].post(options).data
      end
      ...
      def update_authorization(number, options={})
        ...
        # API will reject us if we send a POST request with an empty body.
        options = {:scopes => ""}.merge(options)
        root.rels[:authorizations].patch(options, :uri => { :authorization_id => number }).data
      end
      ...
      def delete_authorization(number)
        root.rels[:authorizations].delete(nil, :uri => { :authorization_id => number }).status == 204
      end
